...
VulkanDemoRenderer::VulkanDemoRenderer()
{
}
...
void VulkanDemoRenderer::frameStart()
{
...
void VulkanDemoRenderer::mainPassRecordingStart()
{
    ...
    VkRect2D scissor = { { int32_t(m_viewport.x()), int32_t(m_viewport.y()) }, { uint32_t(m_viewport.width()), uint32_t(m_viewport.height()) } };
...
void VulkanDemoRenderer::prepareShader(Stage stage)
{
    QString filename;
    if (stage == VertexStage) {
        filename = QLatin1String(":/scenegraph/demo/demo.vert.spv");
    } else {
        Q_ASSERT(stage == FragmentStage);
        filename = QLatin1String(":/scenegraph/demo/demo.frag.spv");
    }
    QFile f(filename);
...
void VulkanDemoRenderer::init(int framesInFlight)
{
